 ...
 */
export function getRouteAuthority(pathname: string, routes: VrpRouteConfig[] = []) {
  let routeAuthority: undefined | string | string[];
  const _getAuthority = (pathname: string, _routes: VrpRouteConfig[]) => {
    _routes.forEach(_route => {
      if (pathname === _route.path) {
        routeAuthority = _route.meta?.authority;
