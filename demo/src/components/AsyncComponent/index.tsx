...
class AsyncComponent extends React.Component<AsyncProps, AsyncState> {
  ...
  async componentDidMount() {
    const { componentInfo, route } = this.props;
    const { layoutStore } = this.injected;

    const { extraInfo } = route;
    ...
    if (typeof componentInfo === 'string') {
      ...
      //   /* webpackChunkName: "[request]" */ `../../../src${componentInfo}`
      // );
      // C = component;
    } else {
      let res = await componentInfo();
